...
public class ContractPrint{
	...
	public void print(Contract contract,String path, HttpServletResponse response) throws Exception{
		...
		//针对模块进行排序操作
	      Collections.sort(oList, new Comparator<ContractProduct>(){  
	            public int compare(ContractProduct c1, ContractProduct c2) {  
	                //排序
	                if(new Integer(c1.getFactoryName()) > new Integer(c2.getId())){  
	                    return 1;  
	                }  
	                if(new Integer(c1.getId()) == new Integer(c2.getId())){  
	                    return 0;  
	                }  
	                return -1;  
	            }  
